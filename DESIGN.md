...
- This browser tile (Sources, the last tile under "Connect an account"): a source tile with `AppWindowIcon` whose status comes from the extension, not the server: "Looking for the extension…", "Not connected", "Extension installed, not connected yet", "Syncing · N items", then "Connected · synced 5 minutes ago · N items" with the check. Without the extension: a default `sm` "Add the extension" link button to the store listing, or, with no listing configured, the "How do I install the extension?" steps. Installed: a default `sm` "Connect this browser" (a spinning `Loader2Icon` while it pairs). Connected: an outline `sm` "Sync now" ("Syncing…" with the spinner, disabled, while a sync runs) and the ghost `ConfirmButton` "Disconnect". A failed sync is one `text-sm text-destructive` line; a failed connect adds `role="alert"`. No motion beyond the spinner.
- Connect a service (`services-section.tsx`, Sources, between "Connect an account" and "Upload an export"; hidden without `features.connectors.services`): a `section.flex.flex-col.gap-3` headed `Kicker as="h2"` "Connect a service", then a lede paragraph (`max-w-[34rem] text-sm leading-relaxed text-muted-foreground`). Connected services are rows of the standard `ul.divide-y.rounded-lg.border.bg-card` (`p-3 flex-col gap-2`): `IconTile` with `PlugIcon`, the name `text-sm font-medium`, one `text-xs text-muted-foreground` line ("12 tools · 9 look things up · 3 take actions"; signed out in `text-destructive`), `ConfirmButton` "Disconnect" at the right; under it, indented `pl-12` to the name, a native checkbox (`size-4 accent-foreground`) labelled `text-xs text-muted-foreground` "Let earcue take actions here when you ask" (only when the service has action tools), and a ghost `sm` "Refresh tools", or an outline `sm` "Reconnect" once signed out. Then a search `Input` (`type="search"`, `bg-card pl-9`, a `SearchIcon` `size-4` inside at the left) over the directory, whose results are the same list shape, six at most, each row: `IconTile` `PlugIcon` (no third-party logos: they would be requests to another host), name, one truncated `text-xs` line "domain · Sign in · what it does", and an outline `sm` "Connect" ("Connected", disabled, once it is). A server that needs a key opens a key form inside its row: a password `Input` "Paste an API key", a narrow `Input` for the header (optional), a default `sm` `h-9` "Connect", and a `Note` that it is stored encrypted. "Connect a server by URL" is a `<details>` like the source tiles' help, with a `type="url"` `Input` and a `sm` `h-9` "Connect" enabled only for an https address. Progress in a `StatusLine`, a failure in `text-sm text-destructive` `role="alert"`, and a closing `Note` crediting integrations.sh and saying actions stay off until turned on. No motion.
...
- Memory view order (`memory-view.tsx`): header, the search form (button "Search") and its answers, "What earcue knows about you", "Ask earcue", "People", then "What earcue has learned". The search stays the first thing on the view; the profile opens the part about what earcue holds.
- Ask earcue panel (replaces the old "Tell earcue something" box, whose example becomes earcue's opening line): a `section.flex.flex-col.gap-3` headed `Kicker as="h2"` "Ask earcue". The conversation is one `ol` with `role="log"` and `aria-live="polite"`, `flex flex-col gap-3`: earcue's lines are plain `text-sm leading-relaxed` prose at the left, the person's are `self-end max-w-[85%] rounded-lg bg-muted px-3 py-2 text-sm` (sunken, no border, no avatar, no name labels). A failed request shows earcue's line in `text-destructive` with `role="alert"`; the person's text stays. Under a reply, its memory changes are a `Chips` row of non-button chips (`chipClass` as an `li`, `max-w-full`): an icon (`BrainIcon` remembered, `XIcon` forgot, `PencilIcon` updated, `LockIcon` for a private memory), then "Remembered: …", "Remembered until 7 Oct: …" (a `once` memory), "Forgot: …" or "Updated: …" truncated, then an `Undo` text button (`font-medium text-foreground underline-offset-4 hover:underline`). An undone chip reads "Undone: …" with no button. Something the turn did in a connected service follows as a second `Chips` row (`aria-label` "What earcue did in your services"), same non-button chips with `PlugIcon`: "Did in Todoist: add task", or "Tried in Todoist: add task (it failed)" in `text-destructive`; no Undo, since earcue cannot reverse another service's change. Below the log: the form, `Input` (`aria-label="Message earcue"`, `maxLength` 2000) plus a default `Button` "Send", both disabled while a reply is pending; a `StatusLine busy` "Looking through your memory…" while it is. Under the form a `Note` says earcue keeps the memories, not the conversation, and, once there is one, a ghost `sm` "New conversation" button. No motion: it is a keyboard path.
- Dashboard view (`dashboard-view.tsx`): header like For you (`ViewTitle` "Dashboard", the lede, an outline Refresh at the right, the shared refresh `StatusLine`). Panels in `grid grid-cols-2 gap-x-3 gap-y-6`, single column under 720px. Each panel is a `section.flex.min-w-0.flex-col.gap-3`: a `min-h-8` header row with `Kicker as="h2"` (the fixed panel title, or the entity's name for a card; a `size-3` `PinIcon` plus `sr-only` "(pinned)" when pinned) and a ghost `icon-sm` `EllipsisIcon` menu (`aria-label` "Options for …": "Pin to the top" or "Unpin", "Hide"). Lists are the standard `ul.divide-y.rounded-lg.border.bg-card` with `p-3` rows: a truncated `text-sm` line and a truncated `text-xs text-muted-foreground` line joined with " · " (no pills; "needs you soon", "stalled" and statuses are words in that line). An entity card is one `Card` (never a card in a card): a `text-xs` activity line, "Talks about …", then `h3 text-xs text-muted-foreground` groups "Open", "What earcue knows", "Latest" (title left, relative time `text-xs text-ink-tertiary` right). Mail and chat statistics are a semantic `table` in `rounded-lg border bg-card overflow-x-auto`, a left-aligned caption, numbers right-aligned `tabular-nums`, and it spans both columns; so does the last panel when it would sit alone in a row. An empty pinned list shows `Empty` copy. Page states: skeletons while loading, `EmptyState` with `LayoutDashboardIcon` ("Add a source" before any source; "Not arranged yet." / "Nothing to show yet." after), and a closing `Note` "Arranged for you 3 hours ago." (or that the standard layout is showing) with a `link` "Show N hidden panels again". Pin and hide change the page at once; hide confirms with a toast. No motion.
...
- People section (`people-section.tsx`, "People"): who the person is in touch with, one row per person in the standard `ul.divide-y.rounded-lg.border.bg-card` (never a card per person), latest contact first, first 8 then an outline `sm` "Show all N". A row is a full-width disclosure `button` (`aria-expanded`, `p-3 text-left`, `hover:bg-accent` over 150ms, rounded only where it meets the list's corners): the name `text-sm font-medium`, then one `text-xs text-muted-foreground` line, "Last in touch 3 Sep · 12 items · Atlas, pricing · 2 memories". Opening a row expands it in place (no motion, no nested card), in this order, each part `flex flex-col gap-2` under `px-3 pb-3`: the addresses it goes by (`font-mono text-xs text-ink-tertiary`, identifiers only); one activity line (`text-xs text-muted-foreground`: last from them, last from you, "usually every N days"); its memories as `MemoryRow`s (`memory-row.tsx`, the same Edit and Forget as the Learned list) in a `ul.divide-y` with `border-t` and no box of its own, private ones behind the same "Show private N" chip; the latest items as `text-sm` lines with a `text-xs text-muted-foreground` date; then the merge control. Loading: `StatusLine busy`; empty: `Empty` "People show up once you add mail or chats." No private memory is counted in the row's line.
- Merge control (inside an open person, the manual merge decision D6 leaves fuzzy matches to): a `FieldLabel` "Same person as", a native `select` styled like `Input` (`h-9 rounded-md border border-input bg-card px-3 text-sm`, `min-w-0 flex-1`) listing the other people by name, then a `ConfirmButton` "Merge" (outline `sm`) whose dialog says the addresses, items and memories become one person and it cannot be undone. The only `select` in the app; reuse this look for any other.
- WhatsApp name question (Sources, inside the WhatsApp tile, above its actions): shown once chats are added and the person has not said which speaker is them. One `text-sm` line "Which one is you in these chats?", then one outline `sm` `Button` per candidate, "I'm Alex Moreno", in a `flex flex-wrap gap-2`; a candidate that matches their own mail name comes first with `variant="default"`. After a choice a `Note` "You're Alex Moreno in these chats." takes the question's place. A failure shows `text-xs text-destructive` with `role="alert"`.
