...
- App shell (`app-shell.tsx` + `sidebar.tsx`): `min-h-dvh grid-cols-[244px_minmax(0,1fr)]`; at `max-[820px]` single column with fixed bottom bar (views + capture pill only; settings link hidden, account moves into sheet).
...
- Press: every button-like element scales `scale(0.96)` on `:active` (never below 0.95); add an opt-out only where motion distracts.
...
4. States: default, hover (gated), focus-visible, active (`scale(0.96)`), loading, empty, error. Verify each by reading code; with a browser, replay motion at 10% speed.
