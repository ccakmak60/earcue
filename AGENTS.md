...
- Knowledge base: `/api/assist/[action]` handles imports (`begin`/`browser`/`items`/`finish` chunked-upload
  ...
  characters, like WhatsApp) with `meta.people` keyed `linkedin:<vanity name>` from the profile URL,
  and the profile, each job application, each post with text and each month of new connections as
  `doc` items. The importer names the archive's owner (the sender whose name is the profile's, else
  the one profile on every conversation) as each block's `meta.self`, and `handleItems` moves that
  alias to the person's own entity (`linkLinkedinSelf()`, `move_alias` with source `confirmed`), so
  open loops can tell their LinkedIn replies from what they were sent. Uploaded documents take the same protocol as `doc` imports
