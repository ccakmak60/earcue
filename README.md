...
Slack, or drop in a WhatsApp, bookmarks, Takeout history or document export) and **Memory** (ask, see what
earcue knows about you, remember, edit, forget). Browser mic/screen capture is on hold: `CAPTURE_ENABLED` in `src/lib/shared/features.ts` hides the
