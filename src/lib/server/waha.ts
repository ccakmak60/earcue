...
export function sessionNameFor(userId: string): string {
...
async function waha(path: string, { method = "GET", body, accept = "application/json" }: { method?: string; body?: unknown; accept?: string } = {}): Promise<WahaJson> {
  const res = await fetch(`${baseUrl()}${path}`, {
    ...
    // A hung WAHA must fail the call, not hold the Worker (or /api/health's poller) open indefinitely.
...
export async function getSession(sessionName: string): Promise<WahaJson> {
  return waha(`/api/sessions/${sessionName}`);
}

function sessionConfig(webhookToken: string, userId: string) {
  return {
...
export async function ensureSession(sessionName: string, webhookToken: string, userId: string): Promise<WahaJson> {
  const config = sessionConfig(webhookToken, userId);
  let existing: WahaJson = null;
  try {
...
export async function getQr(sessionName: string): Promise<WahaJson> {
...
export async function chatsOverview(sessionName: string, limit: number): Promise<WahaJson> {
  return waha(`/api/${sessionName}/chats/overview?limit=${limit}&offset=0`);
}

export async function chatMessages(sessionName: string, chatId: string, sinceSeconds: number, limit: number, offset: number): Promise<WahaJson> {
...
// Output shape matches what insertContextItems() in src/lib/server/knowledge.ts expects.
export function normalizeWahaMessage(msg: WahaJson, chatName?: string | null): ContextItem | null {
