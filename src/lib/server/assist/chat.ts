...
// run as a tool loop. The model looks things up with the five read tools and changes memory with
// three write tools that exist only here. The client keeps the conversation and sends its last
// turns; nothing of it is stored server-side (decision D2), only the memories it changes and the
...
export const CHAT_PROMPT: Prompt = {
  ...
  text:
    ...
    "text of their mail, chats, events and pages), `thread`, `calendar` and `person`. Look something up before saying " +
    ...
    "money, legal or intimate matters.\n" +
...
// What the request itself uses before the loop starts, counted against the loop's subrequest budget:
// the better-auth session (counted as two), the users row, consume() and the profile read.
export const CHAT_PRELUDE_SUBREQUESTS = 5;
...
async function seenMemory(ctx: ToolContext, turn: TurnWrites, ref: unknown): Promise<LiveMemory> {
  ...
  if (id === null) throw new ToolRefused("unseen_ref", "Only a memory ref (m…) that recall or person returned earlier in this conversation turn can be changed. Look it up first.");
...
export function chatWriteTools(run: Run, turn: TurnWrites): Tool[] {
  const remember: Tool = {
    name: "remember",
    ...
    // Embedding fetch and its metering write, the tombstone and near-duplicate lookup, the tombstone
    // delete, the insert or update. No sources to link.
    subrequests: 5,
  ...
  const forget: Tool = {
    ...
    writes: true,
    sensitive: "if_user_asked",
    // Reading the memory for the change chip, the forget statement, marking the profile stale.
    subrequests: 3,
    handler: async (ctx, args) => {
      typed(ctx);
...
export async function runChat(user: { id: string; tz: string | null }, turns: ChatTurn[], { spent = CHAT_PRELUDE_SUBREQUESTS } = {}) {
  ...
  return runLoop(
    {
      run,
      tools: [...READ_TOOLS, ...chatWriteTools(run, turn)],
      messages,
