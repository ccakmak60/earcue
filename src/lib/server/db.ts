...
// across an await boundary longer than the one query that needs it. In the Worker this is
// Hyperdrive-only: `hyperdriveScope()` throws if the binding is missing rather than silently
// falling back to DATABASE_URL, which is what let a stale DSN serve production traffic undetected.
// Exported for auth-server.ts, which needs the same shape wrapped for Kysely.
...
export const sql = async (strings: TemplateStringsArray, ...params: unknown[]): Promise<Rows> => {
  ...
  const client = await openClient(scope.env.HYPERDRIVE.connectionString);
  try {
...
export async function withTransaction(run: (tx: typeof sql) => Promise<void>): Promise<void> {
  ...
  if (scope) {
    client = await openClient(scope.env.HYPERDRIVE.connectionString);
  } else {
    client = pooled = await nodePool().connect();
  }
