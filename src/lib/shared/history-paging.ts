// Paging arithmetic for extension/background.js's history sync, kept here so tests can exercise it.
// The extension imports nothing from src/, so it carries a copy of both functions.
...
export function nextHistoryEnd(page: Visit[], maxResults: number, endTime: number): number | null {
...
export function historyCursor(rowsOldestFirst: Visit[], accepted: number, startTime: number): number {
